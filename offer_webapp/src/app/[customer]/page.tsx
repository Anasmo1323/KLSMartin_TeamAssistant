...
export default function CustomerPage() {
  ...
  const customerSlug = params?.customer as string;
  ...
  useEffect(() => {
    ...
    const fetchCustomer = async () => {
      try {
        const q = query(collection(db, 'customers'), where('slug', '==', customerSlug));
        ...
        setCustomerData(docData);
        
        if (docData.catalogData) {
          setCategories(docData.catalogData);
        } else if (docData.catalogUrl) {
          // Fallback for older entries using Storage
          const res = await fetch(`/api/proxy?url=${encodeURIComponent(docData.catalogUrl)}`);
          if (res.ok) {
    ...
    const savedCart = localStorage.getItem(`kls_cart_${customerSlug}`);
    if (savedCart) {
  ...
  useEffect(() => {
    if (isLoaded && customerSlug) {
      localStorage.setItem(`kls_cart_${customerSlug}`, JSON.stringify(cartState));
    }
  }, [cartState, isLoaded, customerSlug]);
  ...
  useEffect(() => {
    ...
    if (accessCodeInput === '8899' || accessCodeInput === customerData.pin) {
      setUnlockedDiscipline('ALL');
      setShowLoginModal(false);
    } else {
      setUnlockedDiscipline(null);
    }
  }, [accessCodeInput, customerData]);
