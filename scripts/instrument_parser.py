...
EXCEL_PATH = os.path.join(base_dir, "data", "KLS_All_Products.xlsx")
...
# ===========================================================================
# Phase 2.5 -- Deterministic Grouping by xx-xxx
# ===========================================================================

def group_by_code_prefix(df: pd.DataFrame) -> pd.DataFrame:
    """
    Group items strictly by their xx-xxx prefix in the code.
    Calculate the best common family name per group, and push any leftover
    text from the original family extraction into the modifiers field.
    """
    df = df.copy()
    
    # Extract xx-xxx prefix
    df["family_id"] = df["code"].astype(str).str.extract(r"^(\d{2}-\d{3})")[0]
    # Fallback to code if it doesn't match
    df["family_id"] = df["family_id"].fillna(df["code"].astype(str))
    
    grouped = df.groupby("family_id")
    print(f"\n[!] Grouping {len(df)} items into {len(grouped)} prefix families...")
    
    for family_id, group in grouped:
        family_counts = group["family"].dropna().value_counts()
        if not family_counts.empty:
            best_name = family_counts.index[0]
        else:
            best_name = "(Unclassified)"
            
        for idx in group.index:
            old_family = str(df.at[idx, "family"]) if pd.notna(df.at[idx, "family"]) else ""
            old_modifiers = str(df.at[idx, "modifiers"]) if pd.notna(df.at[idx, "modifiers"]) else ""
            
            if old_family and old_family.lower() != best_name.lower():
                # Subtract best_name from old_family to find leftover features
                pattern = re.compile(re.escape(best_name), re.IGNORECASE)
                leftover = pattern.sub("", old_family, count=1).strip(" ,;")
                
                if leftover:
                    if old_modifiers and old_modifiers.lower() != "none":
                        df.at[idx, "modifiers"] = f"{leftover}, {old_modifiers}"
                    else:
                        df.at[idx, "modifiers"] = leftover
                        
            # Apply the definitive best name
            df.at[idx, "family"] = best_name
...
def build_json(df: pd.DataFrame) -> dict:
    ...
    df_work = df.copy()
    if "family_id" not in df_work.columns:
        df_work["family_id"] = df_work["code"].astype(str).str.extract(r"^(\d{2}-\d{3})")[0]
        df_work["family_id"] = df_work["family_id"].fillna(df_work["code"].astype(str))
        
    df_work["family"] = df_work["family"].fillna("(Unclassified)")

    grouped = df_work.groupby("family_id")

    for family_id, group_df in sorted(grouped, key=lambda g: g[0]):
        # Use the most common schema for this family
        schema = group_df["schema"].mode().iloc[0] if len(group_df["schema"].mode()) > 0 else "unknown"
        
        family_name = group_df["family"].iloc[0]
        group_key = f"{family_id} - {family_name}" if family_name != "(Unclassified)" else family_id
...
def run_from_excel(
    ...
    print(f"\n{'=' * 60}")
    print("Phase 2.5 -- Deterministic Grouping")
    print("=" * 60)
    df_clean = group_by_code_prefix(df_clean)
    ...
    excel_out_path = os.path.join(output_dir, "KLS_Product_Families.xlsx")
    df_clean.to_excel(excel_out_path, index=False)
    ...
    print(f"  Saved Excel to: {excel_out_path}")
